...
;(data-table student-tbl)
;(let [h #(map next %)]  (h student-tbl) )
;(let [h #(map next %)]  (data-record (table-keys student-tbl) (h student-tbl) ) )
;(let [keys (table-keys student-tbl)]
;    (map #(data-record keys %) (next student-tbl)))
...
;(get  :id {:surname "Ivanov", :year "1996", :id "1"})
